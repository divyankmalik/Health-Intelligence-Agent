...
class ModelManager:
    ...
    Tier order: PRIMARY (largest/best) → SECONDARY → TERTIARY → FALLBACK.
    Each retry bumps one tier down so a rate-limited model is automatically
    skipped in favour of a smaller but available one.
    """
    ...
    def generate_analysis(self, data, system_prompt, retry_count=0):
        ...
        retry_count doubles as tier index: 0→PRIMARY, 1→SECONDARY, 2→TERTIARY, 3→FALLBACK.
        """
        if retry_count > 3:
