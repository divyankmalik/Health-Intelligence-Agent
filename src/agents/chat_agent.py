...
# RAG (Retrieval-Augmented Generation) pipeline for follow-up chat:
# report text → FAISS vector store → retrieve top-k chunks → feed as context to Groq LLM
class ChatAgent:
    def __init__(self):
        # Lightweight sentence-embedding model; runs locally (no API key needed)
        self.embeddings = HuggingFaceEmbeddings(model_name="all-MiniLM-L6-v2")
        # 200-token overlap prevents losing context at chunk boundaries
    ...
    def _contextualize_query(self, query, chat_history):
        ...
        Needed because FAISS retrieval is context-free — "what does that mean?"
        would return useless chunks unless rephrased as a standalone question.
        """
        if not chat_history:
            return query
